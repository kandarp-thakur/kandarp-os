...
import { sanitizeSlug } from "@packages/utils/slug";
...
export function BlogEditor({ postId }: BlogEditorProps) {
    ...
    const [slugEdited, setSlugEdited] = useState(!!postId);
    const [slugStatus, setSlugStatus] = useState<
        "idle" | "checking" | "available" | "taken"
    >("idle");
    const [slugSuggestion, setSlugSuggestion] = useState<string | null>(null);
    const [slugWasSanitized, setSlugWasSanitized] = useState(false);
    ...
    useEffect(() => {
        ...
        const timer = window.setTimeout(async () => {
            try {
                const query = new URLSearchParams({ slug: form.slug });
                if (postId) query.set("excludeId", postId);
                const res = await fetch(`/api/admin/blog/slug?${query}`);
