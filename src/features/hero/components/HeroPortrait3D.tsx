 ...
 *
 *  - **Lazy loading.** The canvas uses `React.lazy` behind `ClientOnly`, so the
 *    LCP is never blocked and the App Router can preserve the server-rendered
 *    route tree without a `next/dynamic({ ssr: false })` segment bailout.
...
/** Client-only R3F modules, loaded only after the route tree has hydrated. */
const Canvas3D = lazy(() =>
    import("@3d/Canvas3D").then((m) => ({ default: m.Canvas3D })),
);

const AvatarScene = lazy(() =>
    import("@3d/Avatar/AvatarScene").then((m) => ({ default: m.AvatarScene })),
);

const CoderScene = lazy(() =>
    import("@3d/coderModel/CoderScene").then((m) => ({
        default: m.CoderScene,
    })),
);
    ...
    return (
        ...
        >
            ...
            {/* The 3D canvas — lazy, client-only. Transparent so the page
                canvas color shows through (arch §3.5). The ClientOnly boundary
                keeps this progressive enhancement out of the server route tree,
                preventing it from replacing or orphaning routed content. */}
            <ClientOnly>
