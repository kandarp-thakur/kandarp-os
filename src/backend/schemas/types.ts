...
/** A server-side slug contract. Human input is normalized before validation. */
const slug = z.preprocess(
    (value) => (typeof value === "string" ? sanitizeSlug(value) : value),
    z
        .string()
        .min(SLUG_MIN_LENGTH, SLUG_ERROR_MESSAGE)
        .max(
            SLUG_MAX_LENGTH,
            `Slug must be ${SLUG_MAX_LENGTH} characters or fewer.`,
        )
        .refine(isValidSlug, SLUG_ERROR_MESSAGE),
);
...
export const userSchema = z.object({
    ...
    /** Password-reset token digest and expiry; never expose through SafeUser. */
    resetToken: z.string().nullable().default(null),
    resetTokenExp: z.string().nullable().default(null),
    lastLoginAt: z.string().nullable().default(null),
    /** Persistent lockout counters used in addition to per-IP throttling. */
    failedLogins: z.number().int().nonnegative().default(0),
...
/** Typed, CMS-owned Hero configuration persisted in the Settings singleton. */
export const heroStepSchema = z.discriminatedUnion("kind", [
    z.object({ kind: z.literal("command"), text: z.string().min(1).max(200) }),
    z.object({ kind: z.literal("output"), text: z.string().max(1000) }),
    z.object({ kind: z.literal("role") }),
]);

export const heroSchema = z.object({
    eyebrow: z.string().default(""),
    bootBanner: z.string().default(""),
    bootStatus: z.string().default(""),
    ...
    terminal: z
        .object({
            ...
            script: z.array(heroStepSchema).default([]),
            char: z.number().int().min(0).max(5000).default(60),
            pause: z.number().int().min(0).max(30000).default(300),
            read: z.number().int().min(0).max(60000).default(1500),
            roleDwell: z.number().int().min(0).max(60000).default(2500),
            roleCycles: z.number().int().min(1).max(20).default(2),
            loop: z.number().int().min(0).max(120000).default(3000),
            startDelay: z.number().int().min(0).max(30000).default(1100),
        })
        .default({}),
    visual: z
        .object({
            ...
            infinityLoopEnabled: z.boolean().default(true),
            threeEnabled: z.boolean().default(true),
        })
        .default({}),
});
...
export const settingsSchema = z.object({
    ...
    /** Complete editable Hero content and terminal configuration. */
...
/** Generic settings contract. Managed secret fields use dedicated APIs. */
export const safeSettingsSchema = settingsSchema.omit({
    environmentVariables: true,
    integrations: true,
});
export type SafeSettings = z.infer<typeof safeSettingsSchema>;

export function toSafeSettings(settings: Settings): SafeSettings {
    const {
        environmentVariables: _environmentVariables,
        integrations: _integrations,
        ...safe
    } = settings;
    void _environmentVariables;
    void _integrations;
    return safe;
}
...
export const contactSubmissionInputSchema = z.object({
    ...
    // Hidden honeypot. Bots commonly fill every field; legitimate clients leave it empty.
    // The route accepts a value and returns a neutral success without persisting it.
