...
import { canUser } from "@backend/permissions/user-permissions";
import {
    bulkArchive,
    bulkDelete,
    bulkRestore,
...
>(cfg: CrudConfig<T, S>) {
    ...
    async function DELETE(
        ...
        if (session instanceof Response) return session;
        const { id } = await params;
        const ok = await remove(cfg.collection, id, session.sub);
...
>(cfg: CrudConfig<T, S>) {
    async function POST(req: Request) {
        ...
        switch (body.action) {
            case "delete": {
                // Bulk deletion requires the same request-time role + explicit
                // user-override decision as the single-entity delete handler.
                if (!(await canUser(session.sub, session.role, cfg.del)))
                    return error(
