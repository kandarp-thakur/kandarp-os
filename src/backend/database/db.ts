...
// Prevent hot-reload from creating a new client or registering duplicate event
// listeners on every save in development.
const globalForPrisma = globalThis as unknown as {
    prisma?: PrismaClient;
    prismaLoggingAttached?: boolean;
};
...
if (
    ...
    !globalForPrisma.prismaLoggingAttached
) {
    // Wire Prisma's log events into Pino once. Next.js hot reload reevaluates
    // modules while retaining globalThis; without this guard each reload adds
    // another listener and eventually emits MaxListenersExceededWarning.
    prisma.$on("error" as never, (e: unknown) => {
        logger.error({ prisma: true, err: e }, "Prisma error");
    });
    globalForPrisma.prismaLoggingAttached = true;
}
