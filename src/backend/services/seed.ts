...
import { list, create, update } from "@backend/repositories/repo";
...
function seedSettings(): Settings {
    return {
        ...
        favicon: "/icon.svg",
        hero: {
            eyebrow: `${SITE.userAtHost}:~$`,
            bootBanner: "Welcome to Kandarp OS",
            bootStatus: "Boot completed successfully.",
            title: SITE.owner,
            subtitle: "DevOps Engineer",
            description:
                "I build reliable cloud infrastructure, secure networks, automated deployment pipelines, and production-ready systems that keep teams shipping safely.",
            stats: [
            ...
            terminal: {
                ...
                script: [
                    ...
                    {
                        kind: "output",
                        text: "Connection ready. Let's build something.",
                    },
                ],
                char: 60,
                pause: 300,
...
/** Seed the settings singleton and apply idempotent defaults added after install. */
async function seedSettingsIfMissing(): Promise<void> {
    ...
    // The Hero column was introduced after the initial Settings seed. Prisma's
    // database default is intentionally `{}`, so upgrade existing installations
    // once without overwriting any Hero content already saved by an admin.
    const current = existing[0];
    if (current && Object.keys(current.hero ?? {}).length === 0) {
        await update<Settings>("settings", current.id, {
            hero: seedSettings().hero,
        });
    }
}
...
/**
 * Synchronize RBAC system data (roles, permissions, role↔permission links)
 * directly via Prisma — bypassing the repository layer.
 *
...
async function seedRbacIfMissing(): Promise<void> {
    // Always synchronize the matrix. This is an idempotent upgrade path for
    // permissions introduced after an installation's initial seed.
