...
/** Short-lived proof that the password step of a TOTP login succeeded. */
export interface TotpChallenge {
    /** User id authorized to complete the second factor. */
    sub: string;
    /** Whether the final session should use the remember-me lifetime. */
    remember: boolean;
    /** Issued-at (unix seconds). */
    iat: number;
    /** Expiry (unix seconds). */
    exp: number;
    /** Token purpose, preventing confusion with session JWTs. */
    purpose: "totp-login";
}
...
/** Password-verified TOTP challenges expire after five minutes. */
...
/**
 * Create a short-lived, purpose-bound proof for the TOTP login step.
 * The browser never receives a raw user id that it can substitute.
 */
export function signTotpChallenge(userId: string, remember = false): string {
    const now = Math.floor(Date.now() / 1000);
    ...
    return `${body}.${sign(`totp.${body}`)}`;
}

/** Verify a TOTP challenge's signature, expiry, shape, and purpose. */
export function verifyTotpChallenge(token: string): TotpChallenge | null {
    const parts = token.split(".");
    if (parts.length !== 2) return null;
    const [body, signature] = parts as [string, string];
    const expected = sign(`totp.${body}`);
    const sigBuf = Buffer.from(signature);
    const expBuf = Buffer.from(expected);
    if (sigBuf.length !== expBuf.length || !timingSafeEqual(sigBuf, expBuf)) {
        return null;
    ...
    try {
        const payload = JSON.parse(
            Buffer.from(body, "base64url").toString("utf8"),
        ) as Partial<TotpChallenge>;
        const now = Math.floor(Date.now() / 1000);
        if (
            payload.purpose !== "totp-login" ||
        ...
        return payload as TotpChallenge;
    } catch {
        return null;
    }
}
