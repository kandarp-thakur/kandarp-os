...
function toEntity(name: CollectionName, row: Record<string, unknown>): Entity {
    ...
    // DateTime → ISO string. The app-layer schemas model temporal values as
    // strings; include entity-specific DateTime columns as well as audit ones.
    ...
    if (entity.timestamp instanceof Date)
        entity.timestamp = (entity.timestamp as Date).toISOString();

    // FK id fields → app-layer names.
    if (name === "infraEdges") {
        entity.from = entity.fromId;
        entity.to = entity.toId;
        delete entity.fromId;
        delete entity.toId;
    }
...
function stripPrismaRelations(
    ...
    const relFields: Record<CollectionName, string[]> = {
        ...
        // `toEntity()` has already replaced Prisma's Role relation object with
        // the app-layer role machine name. Preserve that scalar: authentication
        // signs it into the session JWT and RBAC requires it for authorization.
        users: ["permissions", "sessions", "refreshTokens", "accounts"],
        settings: [],
        siteCustomization: [],
        analytics: ["user"],
        activityLogs: ["user"],
...
async function fromEntity(
    ...
    // Infrastructure edges retain the app-layer `from` / `to` contract while
    // Prisma stores the endpoint identifiers as `fromId` / `toId`.
    if (name === "infraEdges") {
        if ("from" in out) {
            out.fromId = out.from;
            delete out.from;
    ...
    // Activity logs are append-only and identify the actor through userId.
    // Their Prisma model deliberately omits mutable/provenance audit columns,
    // even though the legacy app-layer Entity contract contains them.
...
/** Pick the requested blog slug or the next available numbered variant. */
async function uniqueBlogSlug(
    requested: unknown,
    excludeId?: string,
): Promise<string> {
    const base = sanitizeSlug(typeof requested === "string" ? requested : "");
    if (!base) return base;
    ...
    return `${base.slice(0, SLUG_MAX_LENGTH - 6)}-999999`;
}
...
export async function create<T extends Entity>(
    ...
    if (name === "blogPosts" && "slug" in input) {
        input.slug = await uniqueBlogSlug(input.slug);
    }
    const payload = await fromEntity(name, input);
    payload.createdAt = now();
    // ActivityLog is an append-only audit record. Unlike mutable entities its
    // Prisma model intentionally has no updatedAt/updatedById columns.
    if (name !== "activityLogs") payload.updatedAt = now();
    if (ARCHIVABLE.has(name)) {
        payload.archivedAt = null;
        payload.archivedById = null;
    }
    // Users are the root of the audit provenance graph, while ActivityLog
    // already records its actor in userId and is immutable. Neither model has
    // createdById/updatedById columns in Prisma.
    if (actorId && name !== "users" && name !== "activityLogs") {
...
export async function update<T extends Entity>(
    ...
    if (name === "blogPosts" && "slug" in input) {
        input.slug = await uniqueBlogSlug(input.slug, id);
    }
    const payload = await fromEntity(name, input);
