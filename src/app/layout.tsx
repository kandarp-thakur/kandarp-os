...
import { BootScreenClient } from "@features/hero/components/BootScreenClient";
...
export default async function RootLayout({
    ...
    return (
        <html lang="en" data-theme={config.theme}>
            ...
            <body
                className={`${fontVariables} relative isolate min-h-screen bg-canvas-base font-sans text-text-primary antialiased`}
            ...
            >
                <Providers>
                    ...
                    {/* Boot Screen — plays once per session, then fades to
                        reveal the site. Mounted here (in the root layout) as a
                        fixed-position sibling of AppShell rather than inside
                        the page's <main>. When it lived inside <main>, its
                        `next/dynamic({ ssr: false })` wrapper forced the entire
                        page route segment to bail out to client-side rendering,
                        discarding the server-rendered section HTML and leaving
                        the page blank. As a layout-level fixed overlay it can
                        no longer affect the page's render strategy. */}
                    <BootScreenClient />
                </Providers>
            </body>
        </html>
