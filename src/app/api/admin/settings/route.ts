...
export async function GET() {
    ...
    return json(toSafeSettings(settings));
}
...
export async function PATCH(req: Request) {
    ...
    const body = await parseBody(req, safeSettingsSchema.partial().strict());
    ...
    if (!updated) return error("Settings not initialized", 404, 404);
    return json(toSafeSettings(updated));
}
