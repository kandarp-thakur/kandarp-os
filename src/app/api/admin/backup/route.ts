...
function sanitizeBackupRows(name: CollectionName, rows: unknown[]): unknown[] {
    if (name === "settings") {
        return rows.map((row) => {
            if (!row || typeof row !== "object") return row;
            const safe = { ...(row as Record<string, unknown>) };
            delete safe.integrations;
            delete safe.environmentVariables;
            return safe;
        });
    }
    return rows;
...
export async function GET() {
    ...
    for (const name of ALL_COLLECTIONS) {
        collections[name] = sanitizeBackupRows(name, await list(name));
    }

    audit(session, "backup.export", "system");
    return json({
        version: 1,
