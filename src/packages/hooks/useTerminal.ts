...
const INITIAL_TERMINAL_LINES: TerminalLine[] = [
    {
        id: "terminal-session-starting",
        kind: "system",
        text: "Starting secure contact session…",
    },
];
...
export function useTerminal(
            ...
            const timer = setTimeout(
                () => {
                    ...
                    setLines((prev) =>
                        index === 0 &&
                        prev.length === 1 &&
                        prev[0]?.id === "terminal-session-starting"
                            ? [line]
                            : [...prev, line],
                    );
    ...
    const clearScreen = useCallback(() => {
        setLines([
            {
                id: `terminal-screen-cleared-${Date.now()}`,
                kind: "system",
                text: "Screen cleared. Type 'help' to list available commands.",
            },
        ]);
        setHistory([]);
