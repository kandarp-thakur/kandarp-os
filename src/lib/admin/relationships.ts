...
import { list, update } from "@/lib/admin/repo";
import type {
    BlogPost,
    CollectionName,
    Experience,
...
export async function syncRelationships(
    ...
    for (const spec of specs) {
        ...
        const targets = list<Row>(spec.targetCollection);
...
export async function cleanupRelationships(
    ...
    if (collection === "blogPosts") {
        const posts = list<BlogPost>("blogPosts");
...
export function resolveRelated(
    ...
    for (const spec of specs) {
        ...
        const all = list<Row>(spec.targetCollection);
