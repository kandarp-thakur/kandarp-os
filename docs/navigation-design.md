...
**One-line vision:** _A navigation that is clean, professional, and intuitive for every visitor — recruiters, clients, and engineers alike — while the DevOps theme lives in the section design, not the labels._
...
box-shadow:
    ...
    /* top light edge */ inset 0 -1px 0 0 rgba(0, 0, 0, 0.04),
    /* bottom shadow edge */ 0 4px 16px rgba(0, 0, 0, 0.06); /* drop shadow */
```
...
| Property    | Default              | Hover        | Active section                      |
| ----------- | -------------------- | ------------ | ----------------------------------- |
| Color       | `#AAB4C5`            | `#FFFFFF`    | `#38BDF8`                           |
| Background  | transparent          | transparent  | transparent                         |
| Font weight | 500                  | 500          | 600                                 |
| Text shadow | none                 | unchanged    | `0 0 12px rgba(56, 189, 248, 0.35)` |
| Transition  | `color, text-shadow` | `180ms ease` | `200ms ease-out`                    |

### 5.3 Active Link Treatment

The current section is identified **only through typography and a restrained
cyan glow**. There is no separate visual indicator: no terminal prompt, cursor,
underline, bottom border, circle, dot, pill background, or floating marker.
The treatment applies consistently to desktop links, dropdown triggers and
rows, mobile links, and the active home wordmark.

### 5.4 Link Rules

- **Active state is automatic** — driven by the navbar's section scroll-spy.
- **Hover changes only color** — links become white with no background or marker.
- **No underline, border, dot, prompt, or active background.**
- **Only `color` and `text-shadow` animate** for active-state changes (`200ms ease-out`).
- **Keyboard accessible** — Tab navigates, Enter activates, focus ring visible.
- **Home is active for the hero section** and uses the same typography-only state.
...
| Active     | `#38BDF8`, weight 600, subtle cyan text glow; no background or marker |
| Hover      | Text changes to `#FFFFFF`; background remains transparent             |
| Transition | Only `color` and `text-shadow`, `200ms ease-out`                      |
...
Scroll-spy uses `IntersectionObserver` to observe every section id and selects
the intersecting section with the greatest visibility ratio. The observer uses
a `0.55` threshold and resolves by **actual document position** (not nav order),
so the correct link highlights even though the nav order (Projects first)
differs from the page order (Experience precedes Projects).

### 9.3 Rules

- **Scroll progress bar is always visible** (except at very top, where it's 0%).
- **Scroll-spy updates active typography automatically** as sections enter view.
- **History navigation synchronizes active state** for hash changes and Back/Forward.
...
| **Active link**    | `aria-current="true"` on the current scroll-spy section link                                 |
...
| Link hover color           | 180ms    | `ease`          |
| Active color + text-shadow | 200ms    | `ease-out`      |
...
4. **Active state is typography only.** Cyan text, weight 600, and a subtle glow; never a prompt, underline, border, dot, circle, marker, or background.
