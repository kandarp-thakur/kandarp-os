...
- **Base logger**: [`logger.ts`](../../src/backend/logging/logger.ts) — configured once per process.
- **Request context**: [`request-context.ts`](../../src/backend/middlewares/request-context.ts) —
  correlation ID + child logger.
- **Request lifecycle**: [`with-logging.ts`](../../src/backend/middlewares/with-logging.ts) —
...
For non-route code (services, repositories), import the root logger:
