...
([`env-schema.ts`](../../src/backend/config/env-schema.ts)). The validated, typed
object (`env`) is the single source of truth — backend modules read from `env`
...
| `MANAGED_SECRETS_KEY`  | string (≥ 32 bytes) | dev fallback           |        ✓        | AES-256-GCM integration credential key |
...
| Variable                | Type                | Default            | Required (Prod) | Notes                                      |
| ----------------------- | ------------------- | ------------------ | :-------------: | ------------------------------------------ |
| `CONTACT_HASH_SECRET`   | string (≥ 32 bytes) | `ADMIN_JWT_SECRET` |        —        | HMAC key for privacy-safe client IP hashes |
...
Contact submissions are always persisted to the database-backed admin inbox. Email
configuration is optional notification delivery and is not the source of truth.
Set a dedicated `CONTACT_HASH_SECRET` when operational key separation is required;
otherwise the validated `ADMIN_JWT_SECRET` is used as the HMAC key. Raw client IP
addresses are never stored with contact submissions.
...
### JWT / Auth / Contact Hash / Managed Secrets Key
...
MANAGED_SECRETS_KEY=<generate a separate value with the command above>
ADMIN_OWNER_EMAIL=admin@kandarp.online
ADMIN_OWNER_PASSWORD=ChangeMe!2026

# ── Contact privacy (optional; falls back to ADMIN_JWT_SECRET) ──
# CONTACT_HASH_SECRET=<generate with the command above>
...
- `MANAGED_SECRETS_KEY` — must be ≥ 32 bytes and remain stable across deploys
...
`MANAGED_SECRETS_KEY` is independent from signing keys and encrypts managed
integration/environment values with AES-256-GCM. Store it in the deployment secret
manager, restrict access, and back it up separately. Changing or losing it makes
existing managed values undecryptable; key rotation therefore requires decrypting
and re-encrypting values through a controlled maintenance operation.
...
| Contact hash key         | `CONTACT_HASH_SECRET`                                                    | Present           |
| GitHub integration       | `GITHUB_TOKEN`                                                           | Present           |

When optional provider variables are absent, the app falls back to local disk for
media, the durable inbox without email notification, `ADMIN_JWT_SECRET` for contact
address hashing, and no GitHub data. These fallbacks do not disable contact
persistence.
