...
**storage abstraction** ([`storage.ts`](../../src/backend/storage/storage.ts)) lets the
app run with local disk (dev) or Cloudinary (prod) — the provider is selected at
module load based on env config.

The **image-optimization pipeline** ([`image-optimization.ts`](../../src/backend/services/image-optimization.ts))
